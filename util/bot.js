...
module.exports = async function () {
  console.clear()
  const spinner = createSpinner(`Bot processing by \u001b[32;1mdavid\u001b[0m`).start({ color: 'green' })
  ...
  client.on("ready", async () => {
    ...
    console.log("\u001b[32m▣ \u001b[38;5;45;1mMusic\u001b[0m Bot By \u001b[38;5;44;1mMr.David\u001b[0m ")
    setInterval(async () => {
      if (await temporary_db.has(`stay`)) {
        let data = await temporary_db.get(`stay`)
        let guild = await client.guilds.cache.get(data.guild_id)
  ...
  client.distube.on("playSong", async (queue, song) => {
    try {
      ...
      await message.edit({ content: `<:__:1103416784587079780>  يتم الآن تشغيل الأغنية :\`${song.name}\` - \`${song.formattedDuration}\`` })
    } catch (e) {
      await queue.textChannel.send({ content: `<:__:1103416784587079780>   يتم الآن تشغيل الأغنية :\`${song.name}\` - \`${song.formattedDuration}\`\nبواسطة: ${song.user}` }).then(async (m) => {
  ...
  client.on("interactionCreate", async (interaction) => {
    if (interaction.type == 2) {
      ...
      if (interaction.commandName == "help") {
        let embeds = [{
          author: { name: "قائمة المساعدة", url: `` },
          author: { name: "الدعم الفني للبوت", url: `https://discord.gg/4fuGKVc5MS` },
          ...
          color: 0x00ff00,
          footer: { text: `Bot By David !#6946 ' Shαdow#1111    ` }
        }]
